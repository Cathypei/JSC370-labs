...
kable(as.data.frame(table(institution)), format = "html", escape = FALSE) %>%
...
kable(as.data.frame(table(schools_and_deps)), format = "html", escape = FALSE) %>%
  kableExtra::scroll_box(width = "100%", height = "400px")
```
...
count_no_abstract <- sum(is.na(abstracts))
```

  - How many of these don't have an abstract? 

_Answer here._ `r count_no_abstract`
...
count_no_title <- sum(is.na(titles))
```

- How many of these don't have a title ? 

_Answer here._ `r count_no_title`
...
kable(database, format = "html", escape = FALSE) %>%
  kableExtra::scroll_box(width = "100%", height = "400px")
```
